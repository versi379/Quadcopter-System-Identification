...
class DenseActor(BasePolicy):
    ...
    def forward(self, obs: th.Tensor, deterministic: bool = False) -> th.Tensor:
        obs_0 = copy.deepcopy(obs)
        obs_0['actor'] = torch.zeros_like(obs_0['actor'])
        obs_0['critic'] = torch.zeros_like(obs_0['critic'])
        mean_actions, log_std, kwargs = self.get_action_dist_params(obs)
        mean_actions_0, log_std_0, kwargs_0 = self.get_action_dist_params(obs_0)
        # Note: the action is squashed
        action = self.action_dist.actions_from_params(mean_actions, log_std, deterministic=deterministic, **kwargs)
        action_0 = self.action_dist.actions_from_params(mean_actions_0, log_std_0, deterministic=True, **kwargs_0)
        # print(action, action_0)
        # print(obs)
        # print(obs_0)
        return action - action_0
...
register_policy("NoBiasMlpPolicy", SACDensePolicy)